...
function is_shortcut(event, shortcut, ignore_mod) {
    ...
    return event_key == shortcut_key && (ignore_mod || event.shiftKey == shortcut.includes('shift') && (event.ctrlKey != global_is_mac && event.metaKey == global_is_mac) == shortcut.match(/ctrl|cmd|command/) && (event.altKey || event.getModifierState?.('AltGraph')) == shortcut.match('/alt|opt/'))
}
